...
export class SummaryComponent implements OnInit {
  ...
  constructor( private orders: OrderService, private lineItems: LineItemService, private cart: CartService,
    private snackBar: MatSnackBar, private router: Router) { }

  ngOnInit(): void {
    this.orders.getOrder(this.cart.orderId, GetOrderParams.cart).subscribe(
  ...
  deleteLineItem(id: string){
    this.lineItems.deleteLineItem(id).pipe(mergeMap(()=> this.orders.getOrder(this.cart.orderId, GetOrderParams.cart))
    ).subscribe( order => {
